...
line_plot_RPM = ggplot(allRPMs,aes(variable, value)) +
  ...
  return_plotList = list(quasirandom_plot,quasirandom_plot_RPM,getConversionRates,line_plot_RPM,linePlot_conversions,fractionTC,fractionAllTC)
  names(return_plotList) = c('quasiRandom_conversion','quasiRandom_RPM','data_conversionRates','linePlot_rpm','linePlot_conversions','fractionTC','fractionAllTC')
...
### maternal
TC3_mgenes = cbind.data.frame(tcReads3TC_Inj_mean[tcReads3TC_Inj_mean$external_gene_name %in% Maternal_write$external_gene_name,] %>%
                                dplyr::select(dplyr::contains("TCReads3")),Maternal_write)
TC3_mgenes = TC3_mgenes %>% dplyr::mutate(fraction3TC = TCReads3_9/totalReads_9) %>% dplyr::select(fraction3TC) %>% dplyr::pull(1)
...
TCreads_M = totalWrite %>% dplyr::filter(type == 'Maternal') %>% dplyr::select(c(allTCReads_9,multipleTC_9,totalReads_9))  %>% 
  dplyr::mutate(fractionAllTC=allTCReads_9/totalReads_9, fractionMultiplTC =multipleTC_9/ totalReads_9) %>% dplyr::select(c(fractionAllTC,fractionMultiplTC))
TCreads_M = cbind.data.frame(TCreads_M,TC3_mgenes)
TCreads_M = reshape2::melt(TCreads_M)

pdf("/Volumes/Macintosh HD/Users/pooja.bhat/Dropbox (VBC)//Paperoutline/figureOutline/paper_ZebrafishSLAMseq/other/rawfigures/figure1//plots/fractionTC_singleAndMultiple.pdf", 
    height=4, width=3)
    ...
    fractionAllAndMultipleTC =   ggpubr::ggviolin(TCreads_M,x='variable',y='value',add='boxplot',fill='red',ylab = 'Fraction T>C reads', ylim=c(0,1),trim = T) + theme_ameres(type='barplot')
    print(fractionAllAndMultipleTC)
...
countingWindows = read.table('/Volumes/Macintosh HD/Users/pooja.bhat/Dropbox/Paperoutline/figureOutline/paper_ZebrafishSLAMseq/other/rawfigures//figure1/data/ConversionRates_allCws.txt',
                             sep="\t",stringsAsFactors = F, header = T)
